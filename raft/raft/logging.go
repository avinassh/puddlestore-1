...
// Debug prints to standard output if VerbosePrint was set and Debug is on, prefaced with time and filename
func (r *RaftNode) Verbose(formatString string, args ...interface{}) {
...
// Prints the given msg as an error if the given condition cond is false.
func AssertWithError(cond bool, format string, args ...string) {
	if !cond {
		Error.Printf(fmt.Sprintf("ASSERTION ERROR: %s", format), args)
	}
}
