...
// STUDENT ADDITION -- ENSURES THAT IT IS MONOTONICALLY INCREASING
// RETURNS TRUE IF TERM IS UPDATED
func (r *RaftNode) setCurrentTerm(newTerm uint64) bool {
	r.ssMutex.Lock()
	defer r.ssMutex.Unlock()

	// STUDENT WRITTEN VERSION OF FUNCTION
	// ----------------------------------------------
	...
	if newTerm < r.stableState.CurrentTerm {
		r.Error("Attempted to set current term from %v -> %v", r.stableState.CurrentTerm, newTerm)
	}
	return false
	// ---------------------------------------------
...
// SHOULD ALWAYS BE CALLED WITH LOGMUTEX LOCKED
...
func (r *RaftNode) getLogEntry(index uint64) *LogEntry {
	if index >= 0 && index < uint64(len(r.logCache)) {
...
// MUST BE CALLED WITH LOGMUTEX LOCKED
func (r *RaftNode) getLastLogIndex() uint64 {
	return uint64(len(r.logCache) - 1)
}

// STUDENT WRITTEN
// logMoreUpdatedThan returns true if node r contains a log that is more
// updated than the one passed in with info in parameters. If both logs
// are equal or r contains less updated, returns false
func (r *RaftNode) logMoreUpdatedThan(theirLastIndex, theirLastTerm uint64) bool {
	r.leaderMutex.Lock()
	defer r.leaderMutex.Unlock()

	myLastEntry := r.getLogEntry(r.getLastLogIndex())
	if myLastEntry == nil {
		// we can't have a more updated log if our log is empty
		return false
	} else if myLastEntry.GetTermId() > theirLastTerm {
		r.Out("Rejected vote because our last log term is larger")
		return true
	} else if myLastEntry.GetTermId() == theirLastTerm {
		if r.getLastLogIndex() > theirLastIndex {
			r.Out("Rejected vote because our log length is larger")
		}
		return r.getLastLogIndex() > theirLastIndex
	}
	return false
}
