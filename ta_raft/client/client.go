...
LOOP:
	for retries < MAX_RETRIES {
		...
		case raft.ClientStatus_OK:
			raft.Debug.Printf("%v is the leader. Client successfully created.\n", remoteNode)
			break LOOP
		case raft.ClientStatus_REQ_FAILED:
			raft.Debug.Printf("Request failed, retrying...\n")
			retries++
		case raft.ClientStatus_NOT_LEADER:
			// The person we've contacted isn't the leader. Use their hint to find
			// the leader.
			raft.Debug.Printf("%v is not the leader, but thinks that %v is\n", remoteNode, reply.LeaderHint)
			remoteNode = reply.LeaderHint
...
func (c *Client) SendRequest(command uint64, data []byte) (*raft.ClientReply, error) {
	...
	for retries < MAX_RETRIES {
		...
		switch reply.Status {
		case raft.ClientStatus_OK:
			raft.Debug.Printf("%v is the leader\n", c.Leader)
			raft.Debug.Printf("Request returned \"%v\"\n", reply.Response)
			return reply, nil
		case raft.ClientStatus_REQ_FAILED:
			raft.Debug.Printf("Request failed: %v\n", reply.Response)
			raft.Debug.Println("Retrying...")
			retries++
		case raft.ClientStatus_NOT_LEADER:
			// The person we've contacted isn't the leader. Use their hint to find
			// the leader.
	...
	return nil, errors.New("maximum number of tries exceeded")
}
