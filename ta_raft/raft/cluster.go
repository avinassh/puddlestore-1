...
import "github.com/brown-csci1380/s18-mcisler-vmathur2/raft/hashmachine"
...
func CreateLocalCluster(config *Config) ([]*RaftNode, error) {
	...
	nodes[0], err = CreateNode(0, nil, new(hashmachine.HashMachine), config)
	if err != nil {
	...
	for i := 1; i < config.ClusterSize; i++ {
		nodes[i], err = CreateNode(0, nodes[0].GetRemoteSelf(), new(hashmachine.HashMachine), config)
		if err != nil {
			return nil, err
		}
	}
...
func CreateDefinedLocalCluster(config *Config, ports []int) ([]*RaftNode, error) {
	...
	nodes[0], err = CreateNode(ports[0], nil, new(hashmachine.HashMachine), config)
	if err != nil {
		Error.Printf("Error creating first node: %v", err)
		return nodes, err
	}
	...
	for i := 1; i < config.ClusterSize; i++ {
		nodes[i], err = CreateNode(ports[i], nodes[0].GetRemoteSelf(), new(hashmachine.HashMachine), config)
		if err != nil {
