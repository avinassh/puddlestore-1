...
// Objects advertised to the tapestry get stored in the location map of the
// object's root node. An object can be advertised by multiple nodes. Objects
// time out after some amount of time if the advertising node is not heard from.
...
// Registers the specified node as having advertised the key. Times out after
...
// Unregisters the specified node for the specified key.
...
// Unregisters all nodes that are registered for the provided key. Returns all
// replicas that were advertising the key.
...
// Get the nodes that are advertising a given key. Now must account for
// the fact that the given key is not advertised, because Fetch may be called iteratively
// on every node in the path due to path caching
func (store *LocationMap) Get(key string) ([]RemoteNode) {
	store.mutex.Lock()
	defer store.mutex.Unlock()

	if results, ok := store.data[key]; ok {
