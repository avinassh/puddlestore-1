...
func (remote *RemoteNode) GetNextHopRPC(id ID) (bool, RemoteNode, error) {
	...
	arg := &IdMsg{id.String()}
	rsp, err := cc.GetNextHopCaller(context.Background(), arg)
	if rsp == nil {
		return false, RemoteNode{}, err
	}
	return rsp.HasNext, rsp.GetNext().toRemoteNode(), remote.connCheck(err)
...
func (remote *RemoteNode) TapestryReplicateRPC(key string, value []byte) error {
	cc, err := remote.ClientConn()
	if err != nil {
		return err
	}
	_, err = cc.TapestryReplicateCaller(context.Background(), &DataBlob{
		Key:  key,
		Data: value,
	})
	return remote.connCheck(err)
