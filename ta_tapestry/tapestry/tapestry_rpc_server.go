...
func (local *Node) GetBackpointersCaller(ctx context.Context, br *BackpointerRequest) (*Neighbors, error) {
	...
	rsp := &Neighbors{remoteNodesToNodeMsgs(bps)}
	return rsp, err
	//__END_TA__
	/* __BEGIN_STUDENT__
	// TODO: students should implement this
...
func (local *Node) TapestryReplicateCaller(ctx context.Context, blob *DataBlob) (*Ok, error) {
	return &Ok{Ok: true}, local.Replicate(blob.Key, blob.Data)
}
